...
# p_isr <- p_isr[which(p_isr$id %in% unique(war$nearest_pow)),]

p_isr_sf <- st_as_sf(p_isr, coords = c("y", "x"), crs = 4326)

p1 + geom_sf(data = p_isr_sf, size = 0.7, alpha = 0.2, pch = 3) + labs(subtitle = "Black crosses are government places")
```
